                ...
                <div class="card-body">
                    ...
                    <div class="basic-form">
                        <form class="form-valide-with-icon needs-validation"
                              action="{{route('add-coupon')}}" method="post">
                            {{csrf_field()}}
                            <div class="row">
                            <div class="row">
                                ...
                                {{--                            <div class="mb-3">--}}
                                {{--                                <label class="text-label form-label" for="validationCustomUsername">Coupon Value</label>--}}
                                {{--                                <div class="input-group">--}}
                                {{--                                    <span class="input-group-text"> <i class="fa fa-list-alt"></i> </span>--}}
                                {{--                                    <input type="text" class="form-control" name="couponValue" id="couponValue"--}}
                                {{--                                           placeholder="Enter coupon value" required--}}
                                {{--                                           value="{{old('couponValue')}}">--}}
                                {{--                                    <div class="invalid-feedback">--}}
                                ...
                                <div class="mb-3 col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="coupon_status" id="coupon_status" data-select2-id="single-select"
                                                tabindex="-1" class="default-select  form-control wide">
                                            <option value="">Choose Status</option>
                                            @foreach($status_arr as $sat)
                                                <option
                                                    value={{$sat->id}}  {{ old('coupon_status') == $sat->id ? "selected" :""}}>{{$sat->status_name_en}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
